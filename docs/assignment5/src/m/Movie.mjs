...
class Movie {
  ...
  static checkReleaseDate(y) {
    ...
    if (!y) {
      ...
      if (typeof y.getMonth !== "function"){
        console.log("checkReleaseDate "+y)
  ...
  set director(p) {
    if (!p) {
      ...
      if (validationResult instanceof NoConstraintViolation) {
        if(this._director) delete this._director.directedMovies[this._movieID];
        // create the new actor reference
        this._director = Person.instances[director_id];
        // add the movie reference to the director
  ...
  addPerson(a) {
    ...
    if (person_id) {
      ...
      if (person_id && validationResult instanceof NoConstraintViolation) {
        ...
        this._actor[person_id] = Person.instances[person_id];
        // add the movie reference to the actor
        this._actor[person_id].actedInMovies[this._movieID] = this;
      } else {
        throw validationResult;
      }
    }
...
Movie.destroy = function (movieID) {
  ...
  if (movie) {
    ...
    if(movie.director) {
      // remove the movie reference from the director
      delete movie.director.directedMovies[movie.movieID];
    }
    for (const person_id of Object.keys(movie.actor)) {
      // remove the movie reference from the actor
      delete movie.actor[person_id].actedInMovies[movie.movieID];
    }
  } else {
